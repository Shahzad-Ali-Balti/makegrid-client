...
export default function ModelViewer({modelPath, height}: { modelPath: string, height:string }) {
        ...
        <div className={cn("w-full rounded-lg relative overflow-hidden", height)}>
            ...
            >
                <Suspense fallback={null}>
                    ...
                    {showShadow &&
						<ContactShadows opacity={0.4} blur={1} scale={10}/>}
                    ...
                    <OrbitControls
                        ...
                        enableZoom={true}
                        enablePan={true}
                        minDistance={1}   // Prevents zooming inside the model
                        maxDistance={20}  // Allows zooming out but not too far
                        dampingFactor={0.05} // Smooth movement
                    />

                    {showGrid && <Grid args={[100, 100]} position={[0, 0, 0]}
					                   cellColor="white"/>}
